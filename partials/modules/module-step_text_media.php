...
<section class="step_text_media pt-24 relative after:absolute after:-z-10 after:right-0 after:top-[50%] after:translate-y-[-50%] after:bg-[url(../assets/images/text_with_media-bg.svg)] after:bg-no-repeat after:bg-right-bottom md:after:w-full md:after:h-[302px] after:bg-contain <?php if(!empty($step_image)) { echo "after:hidden"; } ?>">
  ...
  <div class="container border-b-2 border-[#C4C3BE] border-dashed pb-[100px] <?php if ( empty($step_image) ) { echo "!border-b-0"; } ?>">
    ...
    <?php if(!empty($step_heading)): ?>
      <h2 class="heading-h2 mb-10"><?= $step_heading; ?></h2>
    <?php endif; ?>
    <div class="row flex lg:gap-[100px] md:flex-nowrap flex-wrap gap-10 ">
      ...
      <div class="content-part">
        <?php if(!empty($step_sub_heading)): ?>
          <h3 class="font-semibold text-[30px] mb-[26px]"><?= $step_sub_heading; ?></h3>
        <?php endif; ?>
        <?php if(!empty($step_content)): ?>
          <p class=" mb-[36px] md:max-w-[580px] max-w-full <?php if( empty($step_image) ) { echo "!mt-0 !mb-0 max-w-[760px]"; } ?>"><?= $step_content; ?></p>
        <?php endif; ?>
        <?php
        if( $step_link ):
